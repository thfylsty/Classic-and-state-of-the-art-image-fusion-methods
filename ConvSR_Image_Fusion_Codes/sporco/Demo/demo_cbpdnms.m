% Script demonstrating usage of the cbpdnms function.
%
...
% Pad input to cbpdnms
shp = padarray(sh, [11 11], 'post');
W = ones(size(sh));
W = padarray(W, [11 11], 'post');


% Compute representation using cbpdnms
lambda = 0.01;
opt = [];
opt.Verbose = 1;
opt.MaxMainIter = 500;
...
opt.AutoRho = 1;
opt.StdResiduals = 1;
opt.RhoRsdlTarget = 0.2;
opt.RelStopTol = 1e-3;
opt.AuxVarObj = 0;
opt.RelaxParam = 1.0;
opt.W = W;
opt.HighMemSolve = 1;
[X, optinf] = cbpdnms(D, shp, lambda, opt);
