function [D, optinf] = olcdl_sgd_msk(D0, S, lambda, W, opt)

% online_sgd_rand -- Online Convolutional Dictionary Learning (masked SGD)
%
...
%
% Usage:
%       [D, optinf] = olcdl_sgd_msk(D0, S, lambda, W, opt);
%
% Input:
...
% paramters for masked convolutional sparse coding
lambda_sc = lambda;
opt_sc = [];
opt_sc.Verbose = 0;
...
while k <= opt.MaxMainIter,
  ...
  SampleS = S(:,:,:,index);
  opt_sc.W = Widx;
  [X, ~] = cbpdnms(PzpT(G), SampleS, lambda_sc, opt_sc);
  ...
  gra = nabla_freq(Xf, Gf, Sf, Widx);  % gradient in frequency domain
  Gf = Gf - eta * gra;
...
function g = nabla_freq(Xf, Df, Sf, W)

  residual = sum(bsxfun(@times, Xf, Df), 3) - Sf;
  residual = ifft2(residual, 'symmetric');
  residual = W.* residual;
  residual = fft2(residual);
  g = bsxfun(@times, conj(Xf), residual);
