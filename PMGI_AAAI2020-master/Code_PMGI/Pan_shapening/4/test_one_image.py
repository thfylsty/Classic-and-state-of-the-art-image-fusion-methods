...
for idx_num in range(9,10):
  num_epoch=idx_num
  while(num_epoch==idx_num):

      reader = tf.train.NewCheckpointReader('./checkpoint/CGAN_80/CGAN.model-'+ str(num_epoch))
      ...
      with tf.Session() as sess:
          ...
          for i in range(len(data_ir)):
              ...
              if i<=9:
                  image_path = os.path.join(image_path,'F9_0'+str(i)+".tif")
              else:
                  image_path = os.path.join(image_path,'F9_'+str(i)+".tif")
              end=time.time()
              # print(out.shape)
